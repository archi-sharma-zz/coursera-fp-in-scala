...
class ListsSuite extends FunSuite {
  ...
  test("empty list sum") {
    assert(Lists.sum(List.empty[Int]) === 0)
  }

  test("empty list max") {
    intercept[NoSuchElementException] {
      Lists.max(List.empty[Int])
    }
  }

  test("negative numbers sum") {
    assert(Lists.sum(List(-34, -23, 9)) === -48)
  }

  test("negative numbers max") {
    assert(Lists.max(List(-34, -23, 9)) === 9)
  }

  test("list of zeroes sum") {
    assert(Lists.sum(List(0,0)) === 0)
  }

  test("list of zeroes max") {
    assert(Lists.max(List(0,0)) === 0)
  }

  test("list with repeated elements sum") {
    assert(Lists.sum(List(0, 1, 1, 3, 5, 3)) === 13)
  }

  test("list with repeated elements max") {
    assert(Lists.max(List(0, 1, 1, 3, 3, 3)) === 3)
  }
}
